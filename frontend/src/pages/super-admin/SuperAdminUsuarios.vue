  ...
  <div class="page">
    ...
    <div v-else class="table-card">
      <table class="data-table">
        <thead>
          <tr><th>Nome</th><th>E-mail</th><th>Empresa</th><th>Perfil</th><th>Status</th><th></th></tr>
        </thead>
        <tbody>
          <tr v-for="u in usuarios" :key="u.id">
            ...
            <td><span class="badge-perfil">{{ perfilLabel[u.perfil] }}</span></td>
            <td><span :class="['badge', u.status === 'ativo' ? 'badge--ativo' : 'badge--inativo']">{{ u.status }}</span></td>
            <td><button class="btn-icon" @click="abrirEditar(u)"><i class="pi pi-pencil" /></button></td>
          </tr>
          <tr v-if="!usuarios.length"><td colspan="6" class="empty-state">Nenhum usuário</td></tr>
        </tbody>
      </table>
    </div>
...
.badge-perfil { font-size: 0.8rem; color: var(--color-gold); }
